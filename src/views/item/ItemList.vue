<template>
  ...
  <v-card class="ma-3">
    <v-row >
          <v-col  cols="12" md="10">
            <v-toolbar flat>
            <v-toolbar-title>Item Master : </v-toolbar-title>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "Alias Name", value: "alias_name" },
...
approver_active_show:[],
  ...
  methods: {
    ...
    async loadItemMaste() {
      const result = await api.getItemMasterByCompanyID(localStorage.getItem(server.COMPANYID));
      this.desserts = result.data;
   ...
   async savechangeapproval(){
    if (this.itemchangeapproval.oldapproval == "") {
      ...
      const result = await api.ChangeapprovalUserAccount(this.itemchangeapproval);
