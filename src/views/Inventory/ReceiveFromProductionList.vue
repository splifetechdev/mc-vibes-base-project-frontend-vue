...
export default {
  ...
  data: () => ({
    ...
    dialogedit: false,
    headers: [
      { text: "PD ORD", value: "doc_running_no" },
      ...
      { text: "QTY Receive", value: "receive_qty" },
      { text: "QTY Remain", value: "remain_qty" },
  ...
  methods: {
    ...
    async getPOReceiveData() {
      // alert(localStorage.getItem(server.COMPANYID));
      const result = await api.getAllReceiveFromPOllByCompanyId(
        localStorage.getItem(server.COMPANYID)
      );
      this.desserts = result.data;
      // console.log("desserts:", JSON.stringify(this.desserts));
    ...
    async openeditItem(item) {
      this.props_item = item;
      // console.log("item:", JSON.stringify(item));
      // console.log(`props_item: ${JSON.stringify(this.props_item)}`);

      // this.$router.push(`/receive-from-production-edit/${item.id}`);

      this.$router.push({
        path: `/receive-from-production-edit/${item.id}`,
        query: { json_item: this.props_item },
      });
